...
class Rjadwal extends MY_Controller {
    ...
    {
	     //$Rjadwal = $this->Rjadwal_model->get_all();

 		$data = array(
            'button' => 'Generate',
            'action' => site_url('Rjadwal/calc'),
			'generator' =>set_value('')
		);
        ...
        $this->load->view('Rjadwal/Rjadwal_head');
		$this->load->view('Rjadwal/Rjadwal_list');
		...
		$data['query'] = $this->Rjadwal_model->getschedule($arg);

		$this->load->view('header');
		$this->load->view('Rjadwal/Rjadwal_head',$arg);
		$this->load->view('Rjadwal/Rjadwal_list',$data);
		$this->load->view('footer');
	}
    ...
    public function generate(){
		...
		$data = $this->Rjadwal_model->get_by_drivertanggal($data);
		$combo['query']= $this->Master->getdriver("");

		$this->load->view('header',$combo);
		$this->load->view('Rjadwal/Rjadwal_head',$datahead);
		$this->load->view('Rjadwal/Rjadwal_list',$data);
        $this->load->view('footer');
	...
	foreach ($this->Rjadwal_model->get_all() as $data) {
            $kolombody = 0;
    ...
    public function word()
    {
        ...
        header("Content-Disposition: attachment;Filename=Rjadwal.doc");

        $data = array(
            'Rjadwal_data' => $this->Rjadwal_model->get_all(),
            'start' => 0
        );

        $this->load->view('Rjadwal/Rjadwal_doc',$data);
    }

}

/* End of file Rjadwal.php */
/* Location: ./application/controllers/Rjadwal.php */
